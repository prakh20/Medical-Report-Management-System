...
<form id="a" action="" method="POST">
  ...
  <div class="form-row">
    <div class="form-group col-md-6">
      <label>Address</label>
      <input type="text" class="form-control" id="address" name="address" placeholder="Enter new address..." required>
    </div>
    <div class="form-group col-md-6">
      <label>Phone</label>
...
<?php
 if(isset($_POST['btn'])) 
 {
   ...
   if((!empty($_POST['email']))  && (!empty($_POST['password'])) && (!empty($_POST['address'])) && (!empty($_POST['phone'])))
   {
   $email = $_POST['email'];
   ...
   $sql = "UPDATE patient SET email = '$email',password = '$password',address = '$address',phone = '$phone' WHERE pid=$id" ;
