...
<table class="table">
     ...
     { 
       ...
       if($selected=="RDoctorId")
      {
          ...
          if( mysqli_num_rows( $selectRes )==0 )
          {
              ...
              {
                echo "<tr><td>{$row['did']}</td><td>{$row['dname']}</td><td>{$row['hid']}</td><td>{$row['hname']}</td><td>{$row['address']}</td><td>{$row['date']}</td><td>{$row['report_type']}</td><td>{$row['report_name']}</td><td>{$row['report_description']}</td><td><input type='submit' value='Download'></td></tr>\n";
              }
        }
     }
  ...
  elseif($selected=="TDoctorName")
  {
      ...
      if( mysqli_num_rows( $selectRes )==0 )
      {
          ...
          while( $row = mysqli_fetch_assoc( $selectRes ) )
          {
            echo "<tr><td>{$row['did']}</td><td>{$row['dname']}</td><td>{$row['hid']}</td><td>{$row['hname']}</td><td>{$row['address']}</td><td>{$row['date']}</td><td>{$row['test_type']}</td><td>{$row['test_name']}</td><td>{$row['test_description']}</td><td><input type='submit' value='Download'></td></tr>\n";
          }
    }
 }
